...
def phasecorr(
    ...
    *,
    ...
    window = 2 * lcorr + 2 * lpad + 1

    if single_image:
        # Y is (nimg, nb, ly, lx) -> (1, nb, ly, lx)
        # For each block, extract the center window
        cc0 = np.empty((nb, window, window), dtype=np.float32)
        for n in range(nb):
            ly, lx = Y.shape[2], Y.shape[3]
            cy, cx = ly // 2, lx // 2
            y1 = cy - window // 2
            y2 = y1 + window
            x1 = cx - window // 2
        ...
        cc2 = [
            c2.reshape(nb, 1, window, window)  # nimg=1
            for c2 in cc2
        ]
